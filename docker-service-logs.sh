...
# Gerekli değişkenler
SERVICE_NAME=$1
DOCKER_MASTER="$DOCKER_MASTER_USER@$DOCKER_MASTER_HOST"
...
CONTAINER_ID=$(ssh $SSH_OPTS root@$NODE "docker ps --filter 'name=$SERVICE_NAME' --format '{{.ID}}' | head -n 1")
...
# Get Logs
ssh -t $SSH_OPTS root@$NODE "docker logs $CONTAINER_ID --tail 200 -ft"
